...
export type ProjectHostToWebview = { type: 'project:load'; project: any; viewState: ViewState; prefs: Record<string, unknown>; services: Record<string, any>; isConnected: boolean; isSubscribed?: boolean; statuses?: Record<string, TaskStatus>; serverHost?: string; isReadonly?: boolean; voiceStatus?: { enabled: boolean; deepgramConfigured: boolean; plannerConfigured: boolean; errors: string[]; model?: string } } | { type: 'project:update'; project: any } | { type: 'project:services'; services: Record<string, any> } | { type: 'project:validateResponse'; requestId: number; result: any; error?: string } | { type: 'project:dirtyState'; isDirty: boolean; isNew: boolean } | { type: 'project:initialState'; state: ViewState } | { type: 'project:initialPrefs'; prefs: Record<string, unknown> } | { type: 'voice:deepgramTokenResponse'; requestId: number; key?: string; error?: string } | { type: 'voice:generateProjectEditResponse'; requestId: number; project?: any; summary?: string; error?: string } | { type: 'voice:externalUtterance'; transcript: string } | { type: 'shell:init'; theme: Record<string, string>; isConnected: boolean } | { type: 'shell:themeChange'; tokens: Record<string, string> } | { type: 'shell:connectionChange'; isConnected: boolean } | { type: 'shell:viewActivated'; viewId: string } | { type: 'shell:event'; event: unknown };

/** All messages the ProjectWebview can send to the extension host. */
export type ProjectWebviewToHost = { type: 'view:ready' } | { type: 'view:initialized' } | { type: 'project:contentChanged'; project: any } | { type: 'project:validate'; requestId: number; pipeline: any } | { type: 'project:requestSave' } | { type: 'project:viewStateChange'; viewState: ViewState } | { type: 'project:prefsChange'; prefs: Record<string, unknown> } | { type: 'project:openLink'; url: string; displayName?: string } | { type: 'voice:deepgramToken'; requestId: number } | { type: 'voice:generateProjectEdit'; requestId: number; transcript: string; currentProject: any; services: Record<string, any> } | { type: 'voice:openExternalCapture' } | { type: 'voice:metric'; event: Record<string, unknown> } | { type: 'status:pipelineAction'; action: 'run' | 'stop' | 'restart'; source?: string } | { type: 'trace:clear' };
